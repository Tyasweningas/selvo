...
// import { thumbnail_1 } from "@/assets/items";
...
interface Props {
  item?: ProductCardType; // item dibuat optional untuk mencegah crash
}

export default function ProductCard({ item }: Props) {
  // Prevent SSR crash
  if (!item) {
    console.warn("ProductCard menerima item undefined");
    return null;
  }
...
<div className="relative bg-bg-nav w-[250px] h-95 rounded-2xl overflow-hidden">
  {/* Kategori badge */}
  <div
    className={`absolute top-2 left-2 px-3 py-1 text-sm rounded-full text-black font-semibold ${
      categoryColor[item.categoryId] || "bg-gray-300"
    }`}
  >
  ...
  <div className="flex flex-col items-center pt-6 pb-2 px-4">
    ...
    <div className="w-full mt-3 text-left"> {/* ← ini kuncinya */}
